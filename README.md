...
4. JPA 트랜젝션 처리를 통하여 RuntimeException 시 데이터 롤백이 되도록 처리하였습니다.<br><br>
...
7. 게시판 API 중, 특정 게시글을 클릭했을 경우 게시글의 조회수가 1 올라가는 API 의 경우,<br>
   ...
   서버 복제시 올바른 동작을 안 하는 문제가 있으므로, 데이터베이스 락을 고려할 수도 있지만,<br>
   저는 Redis 의 싱글 스레드 처리 규칙과 메모리 사용을 통한 빠르고 좋은 성능을 고려하여 Redis 공유락을 만들어 적용하였습니다.<br>
   구현 방식은 따로 설명드리지 않지만 패턴화하여 동일한 방식으로 간편하게 사용이 가능하도록 처리를 하였고,<br>
   테스트를 위하여 은행 계좌 동시 입출력 테스트 샘플을 만들어 올바른 동작에 대한 검증을 끝냈습니다.<br>
   결론적으로, 게시글 조회수 1up api 는 불특정 다수가 동시 요청을 할 가능성이 큰 취약 영역이므로 락을 걸었으며,<br>
   또한 당장 처리되어야 하는 종류의 api 가 아니기에 스레드 풀을 사용하여 처리되는 시간을 응답 시간의 뒤로 미루어 효율을 높였습니다.<br><br>
...
9. 인증/인가 방식은 JWT 를 사용하였습니다.<br>
   ...
   서버 메모리 내에서 회원 정보를 저장할 필요가 없기에 호스트 입장에서 경제적이라는 장점 때문입니다.<br>
   간혹 JWT 를 구현하시면서 Redis 에 로그인 회원 정보를 저장하는 방식으로 구현하시는 것도 많이 보았지만,<br>
...
위와 같이 게시판 프로젝트를 하나 만드는 데에 어디까지 고려할 수 있는지,<br>
그에 대해 정리하였고, 본 프로젝트는 이와 같은 사항들을 반영하여 작성된 결과물입니다.<br>

서비스를 이용하는 고객의 입장에서 게시판 서비스란 코웃음이 날 정도로 단순한 서비스일 뿐이겠지만,<br>
서버 개발자의 입장에서는 겉으로 보이는 기능 구현을 넘어서 완성도 높은 솔루션을 구현하기 위하여 고려해야 할 사항이 눈에 보이는 것 이상으로 많다는 것을 보여주는 예시로,<br>

이러한 지식을 갖추고 실천하는 것이 중요한 이유는,<br>
개발에 대한 심도 깊은 지식과 올바른 구조를 바탕으로 진행할 수 있는 능력이 있어야만<br>
단순한 게시판 서비스를 넘어 보다 복잡하고 중요한 서비스에 대하여 안정적으로 처리가 가능할 것이며,<br>
이를 고려하지 않은 상황에서는 당장 개발 요청자의 요구에는 응할 수 있겠지만 추후 해당 프로젝트를 이어갈 다른 개발자들과의 마찰이나 근본적인 개발 기술력에 대한 문제에 맞딱드릴 것이기 때문입니다.<br>

제가 개발을 시작한 시점의 개발 트랜드와 현재의 트랜드를 비교하였을 때 굉장히 많은 변화가 있어왔고,<br>
앞으로도 있을 것이라고 생각합니다.<br>

그때마다 더 좋은 방식이 있다면, 해당 방식을 흡수하며 본 프로젝트 역시 변경을 해야할 것이지만,<br>

근본적으로는 DDD(Domain Driven Design) 를 기반으로 한 서비스 도메인 설계 방법론과<br>
MSA 를 통한 마이크로 서비스간의 느슨한 결합은, 추후 라이브러리나 코딩 스타일이 어떻게 변하든 기존 서비스에 대해서는 영향을 끼치지 않는 안정적인 구조를 보장할 것이라는 정리를 마지막으로 본 프로젝트의 설명글을 마칩니다.