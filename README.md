...
개발 실력을 어떻게 해야 최대한 심플하고 인상적으로 표현할 수 있을 지에 대해 고민하였습니다.<br>
...
1. 사용 기술 소개 :<br>
   저는 본 프로젝트를 구현하기 위하여 Kotlin, JPA(QueryDSL), Retrofit2, Kafka, Redis, Swagger, Spring Security 를 사용하였습니다.<br>
   Kotlin 을 사용한 이유는, Java 의 기본 철학을 그대로 가져가면서, Java 와 상호 호환이 가능하며,<br>
   Java 에서 제공하지 않는 여러 유용한 기능들을 제공하기 때문입니다.<br>
   대표적으로, Kotlin 에서는 Java 진영에서 사용하는 LOMBOK 을 사용하지 않고도 해당 기능을 언어 자체적으로 지원하며,<br>
   특히 Kotlin 의 Nullable, Not Nullable 변수에 대한 제약은 언어 자체적으로 Null Safety 를 보장합니다.<br>
   JPA 를 사용한 이유는, JDBC 나 Mybatis 와 같은 제가 사용해본 여타 데이터베이스 라이브러리보다 확실히 편리하고 보편적인 ORM 이기 때문이며,<br>
   네트워크 요청 라이브러리로 자주 사용되는 Rest Template 나 Web Client 를 놔두고 Retrofit2 를 사용한 이유는,<br>
   해당 라이브러리가 안드로이드 진영에서 자주 사용되던 것이기에 안드로이드 개발과 병행하던 시절에 혼용해 사용하기 위해서 선택했던 것이며,<br>
   이러한 이유를 제외하고도, Retrofit2 는 OKHTTP 를 래핑하여 모바일 환경을 위해 만들어진 라이브러리이므로 빠르고 가볍다는 장점도 있기에 사용중입니다.<br>
   Swagger 는 클라이언트에 대한 문서화 외에도, 개발시 작성한 API 컨트롤러에 대하여 설명을 강제하기에 따로 주석을 작성하지 않아도 코드 정리가 된다는 장점으로 사용하고 있으며,<br>
   Spring Security 는 인증/인가 처리에 대하여 패턴화하여, 복잡할 수 있는 인증/인가 처리를 간편하게 적용이 가능하기에 사용하였습니다.<br>
   최근 MSA 를 위해 자주 사용되는 Kafka, Redis 에 대한 설명은 제외하겠습니다.<br><br>
